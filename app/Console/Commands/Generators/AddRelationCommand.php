        ...
        foreach ($modelNames as $modelName) {
            ...
            foreach ($columns as $column) {
                ...
                if (preg_match('/^(.*_image)_id$/', $columnName, $matches)) {
                    ...
                    if (!array_key_exists($relationName, $existingRelations[$modelName])) {
                    ...
                    if (!array_key_exists($reverseRelationName, $existingRelations[$className])) {
            ...
            if (count($relationNames) === 2) {
                if (($modelName == ucfirst($relationNames[0]).ucfirst(\StringHelper::pluralize($relationNames[1]))) || ($modelName == ucfirst($relationNames[1]).ucfirst(\StringHelper::pluralize($relationNames[0])))) {
                    ...
                    if (!array_key_exists($relationName, $existingRelations[$child])) {
                        $newRelations[$modelName][$relationName] = [
                            'type'       => 'BelongsToMany',
                    ...
                    if (!array_key_exists($relationName, $existingRelations[$parent])) {
                        $newRelations[$modelName][$relationName] = [
                            'type'       => 'BelongsToMany',

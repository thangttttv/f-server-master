...
class NotificationRepository extends SingleKeyModelRepository implements NotificationRepositoryInterface
{
    ...
    public function getByAdvertiserId($advertiserId, $order = 'id', $direction = 'desc', $offset, $limit)
    {
        $model = $this->getBlankModel();

        return $model->where('advertiser_id', '=', $advertiserId)->orWhere(function ($query) {
            $query->where('advertiser_id', '=', Notification::BROADCAST_USER_ID)->where('locale', '=', \App::getLocale());
        })->orderBy($order, $direction)->offset($offset)->limit($limit)->get();
    }
    ...
    public function countByAdvertiserId($advertiserId)
    {
        $model = $this->getBlankModel();
